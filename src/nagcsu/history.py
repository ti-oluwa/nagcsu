...
module, `guess_column_map` matches that documented layout with a
...
(see `nagcsu.config.HistoryConfig`) if a workbook does not match.
"""
...
def load_observed_history(
    ...
    *,
    ...
    each entry in `nagcsu.objective.SCORED_FIELD_VECTORS` (`FPR`,
    `FWCT`, `FGOR`), so it can be passed straight to
    `nagcsu.objective.score` alongside a res2df-produced simulated
    frame. The date column is parsed and normalized to midnight the same
    way `nagcsu.summary.load_summary` normalizes its own `DATE`
    column, since `objective.score` merges the two frames on an exact
    date match. Field-total water cut and GOR are computed as simple
    ...
    :param column_map: Explicit res2df-vector-name to workbook-column
        mapping. Falls back to `guess_column_map` for anything
