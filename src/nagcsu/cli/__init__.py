...
`nagcsu sensitivity run`, and `nagcsu report` (`list`, `show`). Each
subcommand lives in its own module under this package; this module only
assembles them under the top-level group and resolves the shared
`--config` option every subcommand reads through
:func:`nagcsu.cli._context.load`.
"""
